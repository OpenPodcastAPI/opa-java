...
public class SubscriptionService {
    ...
    /// Fetches a single subscriptionEntity for an authenticated user, if it exists
    ///
    /// @param subscriptionUuid the UUID of the subscriptionEntity
    /// @param userId           the database ID of the user
    /// @return a [SubscriptionDTO.UserSubscriptionDTO] of the user subscriptionEntity
    ...
    public SubscriptionDTO.UserSubscriptionDTO getUserSubscriptionBySubscriptionUuid(UUID subscriptionUuid, Long userId) {
        log.debug("Fetching subscriptionEntity {} for user {}", subscriptionUuid, userId);
        UserSubscriptionEntity subscription = userSubscriptionRepository.findByUserIdAndSubscriptionUuid(userId, subscriptionUuid)
                .orElseThrow(() -> new EntityNotFoundException("subscriptionEntity not found for user"));

        log.debug("SubscriptionEntity {} for user {} found", subscriptionUuid, userId);
    ...
    /// Persists a new user subscriptionEntity to the database
    /// If an existing entry is found for the user and subscriptionEntity, the `isSubscribed` property is set to `true`
    ///
    ...
    protected SubscriptionDTO.UserSubscriptionDTO persistUserSubscription(SubscriptionEntity subscriptionEntity, Long userId) {
        ...
        UserSubscriptionEntity newSubscription = userSubscriptionRepository.findByUserIdAndSubscriptionUuid(userId, subscriptionEntity.getUuid()).orElseGet(() -> {
            log.debug("Creating new user subscriptionEntity for user {} and subscriptionEntity {}", userId, subscriptionEntity.getUuid());
    ...
    public SubscriptionDTO.BulkSubscriptionResponseDTO addSubscriptions(List<SubscriptionDTO.SubscriptionCreateDTO> requests, Long userId) {
        ...
        for (SubscriptionDTO.SubscriptionCreateDTO dto : requests) {
            try {
                // Fetch or create the subscriptionEntity object to subscribe the user to
    ...
    /// Updates the status of a subscriptionEntity for a given user
    ///
